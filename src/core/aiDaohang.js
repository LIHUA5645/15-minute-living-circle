...
export function shiDaoHangYiTu(wen) {
  ...
  // 明确的导航指令词（「我想去广西博物馆」「帮我导航回家」这类没有类别词的也算，交给选点层解释为什么去不了/去哪）
  return /导航|我想去|我想要去|我要去|我想到|带我|送我去|开车去|骑车去|坐车去|想去/.test(s);
}

// 从导航语句里抽取目的地名称：剥掉客套词与动作词，剩下的就是地名
// 「我想去广西博物馆」→「广西博物馆」；「帮我导航回家」→「回家」
export function tiQuMuDiDi(wen) {
  let s = String(wen || '').trim();
  s = s.replace(/[。！！？?，,～~\s]+$/, ''); // 去尾部标点
  s = s.replace(/(帮我|麻烦|请你|我想|我要|我打算|我准备|我想着|我想|打算|准备)/g, '');
  s = s.replace(
