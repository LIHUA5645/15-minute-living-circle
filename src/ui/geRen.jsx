...
export function GeRen({ open, onClose, yongHu, onTuiChu, onHuiKan }) {
  ...
  const pingJun = woDe.length ? Math.round(woDe.reduce((s, r) => s + (r.total || 0), 0) / woDe.length) : null;

  // 导出我的体检档案为 JSON 文件
  function daoChu() {
    const neirong = JSON.stringify({ zhangHao, daoChuShiJian: new Date().toISOString(), jiLu: woDe }, null, 2);
    ...
    <div className="admin-mask" onClick={onClose}>
      <div className="admin-panel" style={{ width: 480 }} onClick={(e) => e.stopPropagation()}>
        <div className="admin-head">
          ...
          <button className="a-btn a-btn-ghost a-btn-sm" onClick={onTuiChu} title="退出当前账号">退出登录</button>
        ...
        <div className="admin-body">
          <div className="gr-ka">
            ...
            <div className="gr-xin">
              ...
              <span>
                ...
                {yongHu.zhuCeShiJian ? ` · 注册于 ${new Date(yongHu.zhuCeShiJian).toLocaleDateString()}` : ''}
              </span>
            </div>
          ...
          <div className="gr-tongJi">
            <div className="gr-xiang"><b>{woDe.length}</b><span>累计体检</span></div>
            <div className="gr-xiang"><b>{zuiGaoFen || '—'}</b><span>历史最高分</span></div>
            <div className="gr-xiang"><b>{pingJun ?? '—'}</b><span>平均得分</span></div>
            <div className="gr-xiang"><b>{zuiXin ? zuiXin.mang : '—'}</b><span>最近盲区</span></div>
          </div>
            ...
            <div className="gr-fenBu">
              {fenBu.map(({ d, shu }) => (
                <div key={d} className="gr-fenBu-xiang">
                  ...
                  <i><u style={{ width: `${woDe.length ? Math.round((shu / woDe.length) * 100) : 0}%` }} /></i>
                  <em>{shu} 次</em>
                </div>
              ))}
            </div>
          ...
          <div className="a-field">
            <div className="sec-title">最近体检记录{woDe.length > 0 && <span className="sec-tag">点「回看」在地图上打开该社区</span>}</div>
            <div className="a-history" style={{ maxHeight: 190 }}>
              {woDe.length === 0 && (
                <div className="empty-tip">还没有体检记录。选好社区中心点后点「开始体检」，完成后自动归档到这里。</div>
              )}
          ...
          {gaiMiMaKai ? (
            <div className="gr-gaiMiMa">
              ...
              <div className="a-row">
                ...
                <button className="a-btn a-btn-ghost" onClick={() => { setGaiMiMaKai(false); setGaiCuo(''); }}>取消</button>
              </div>
            </div>
          ) : (
            <div className="a-row">
              <button className="a-btn a-btn-ghost" onClick={() => setGaiMiMaKai(true)}>修改密码</button>
              <button className="a-btn a-btn-ghost" onClick={daoChu} disabled={!woDe.length}>导出档案</button>
              <button className="a-btn a-btn-ghost a-btn-weiXian" onClick={qingKong} disabled={!woDe.length}>清空记录</button>
            </div>
          )}
